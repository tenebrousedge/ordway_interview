...
class SeatsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create seat" do
    assert_difference('Seat.count') do
      post seats_url, params: { seat: { column: @seat.column, row: @seat.row, status: @seat.status } }
    end
  ...
  test "should update seat" do
    patch seat_url(@seat), params: { seat: { column: @seat.column, row: @seat.row, status: @seat.status } }
